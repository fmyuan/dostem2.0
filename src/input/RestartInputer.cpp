...
void RestartInputer::init(string & dirfile){
	...
	fnonvascularV=restartFile->get_var("FNONVASCULAR");
...
void RestartInputer::getRestartData(RestartData *resid, const int &cid){
	...
	ifwoodyV->set_cur(cid);
	varbool = ifwoodyV->get(&resid->ifwoody, 1);
	if(!varbool){
	...
	ifdeciwoodyV->set_cur(cid);
	varbool = ifdeciwoodyV->get(&resid->ifdeciwoody, 1);
	if(!varbool){
	...
	ifperenialV->set_cur(cid);
	varbool = ifperenialV->get(&resid->ifperenial, 1);
	if(!varbool){
	...
	nonvascularV->set_cur(cid);
	varbool = nonvascularV->get(&resid->nonvascular, 1);
	if(!varbool){
	...
	fnonvascularV->set_cur(cid);
	varbool = fnonvascularV->get(&resid->fnonvascular, 1);
	if(!varbool){
		string msg = "problem in reading 'FNONVASCULAR' in RestartInputer";
 		cout<<msg+"\n";
 		exit(-1);
	}

	vegageV->set_cur(cid);
	varbool = vegageV->get(&resid->vegage, 1);
	if(!varbool){
	...
	vegcovV->set_cur(cid);
	varbool = vegcovV->get(&resid->vegcov, 1);
	if(!varbool){
	...
	varbool = rootfracV->get(&resid->rootfrac[0], 1, MAX_ROT_LAY);
	if(!varbool){
	...
	vegwaterV->set_cur(cid);
	varbool = vegwaterV->get(&resid->vegwater, 1);
	if(!varbool){
	...
	vegsnowV->set_cur(cid);
	varbool = vegsnowV->get(&resid->vegsnow, 1);
	if(!varbool){
	...
	vegcV->set_cur(cid, 0);
	varbool = vegcV->get(&resid->vegc[0], 1, NUM_PFT_PART);
	if(!varbool){
	...
	strnV->set_cur(cid, 0);
	varbool = strnV->get(&resid->strn[0], 1, NUM_PFT_PART);
	if(!varbool){
	...
	labnV->set_cur(cid);
	varbool = labnV->get(&resid->labn, 1);
	if(!varbool){
	...
	deadcV->set_cur(cid);
	varbool = deadcV->get(&resid->deadc, 1);
	if(!varbool){
	...
	deadnV->set_cur(cid);
	varbool = deadnV->get(&resid->deadn, 1);
	if(!varbool){
	...
	toptV->set_cur(cid);
	varbool = toptV->get(&resid->topt, 1);
	if(!varbool){
	...
	eetmxV->set_cur(cid);
	varbool = eetmxV->get(&resid->eetmx, 1);
	if(!varbool){
	...
	growingttimeV->set_cur(cid);
	varbool = growingttimeV->get(&resid->growingttime, 1);
	if(!varbool){
	...
	unnormleafmxV->set_cur(cid);
	varbool = unnormleafmxV->get(&resid->unnormleafmx, 1);
	if(!varbool){
	...
	toptAV->set_cur(cid, 0);
	varbool = toptAV->get(&resid->toptA[0], 1, 10);
	if(!varbool){
	...
	eetmxAV->set_cur(cid, 0);
	varbool = eetmxAV->get(&resid->eetmxA[0], 1, 10);
	if(!varbool){
	...
	growingttimeAV->set_cur(cid, 0);
	varbool = growingttimeAV->get(&resid->growingttimeA[0], 1, 10);
	if(!varbool){
	...
	unnormleafmxAV->set_cur(cid, 0);
	varbool = unnormleafmxAV->get(&resid->unnormleafmxA[0], 1, 10);
	if(!varbool){
	...
	varbool = foliagemxV->get(&resid->foliagemx, 1);
	if(!varbool){
