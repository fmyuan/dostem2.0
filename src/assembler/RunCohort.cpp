	       ...
	       if (outputyrind >=0) {
	    	   if (md->outSiteDay){
	    		   for (int id=0; id<dinmcurr; id++) {
	    			   ...
	    			   envdlyouter.outputCohortEnvVars_dly(&cht.outbuffer.envoddlyall[id],
	    	   ...
	    	   if (md->outSiteMonth){
	    		   ...
	    		   envmlyouter.outputCohortEnvVars_mly(&cht.cd.m_snow, &cht.ed,
	    				                               icalyr, im, mstepcnt);
		    	   bgcmlyouter.outputCohortBgcVars_mly(&cht.bd, cht.fd,
		    			                               icalyr, im, mstepcnt);
	    	   ...
	    	   if (md->outSiteYear && im==11){
	    		   ...
	    		   envylyouter.outputCohortEnvVars_yly(&cht.cd.y_snow, &cht.ed, icalyr, ystepcnt);
		    	   bgcylyouter.outputCohortBgcVars_yly(&cht.bd, cht.fd, icalyr, ystepcnt);

		    	   if (cht.cd.y_veg.vegcov>0.){
	    		    		envylyouter.outputCohortEnvVars_yly(&cht.cd.y_snow, &cht.ed,
	    		    				icalyr, ystepcnt);
	    		    		bgcylyouter.outputCohortBgcVars_yly(&cht.bd, cht.fd,
	    		    				icalyr, ystepcnt);
