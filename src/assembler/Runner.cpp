...
void Runner::initInput(const string &controlfile, const string &runmode, const int &nthproc){
...
//output setting-up
void Runner::initOutput(const int &nthproc) {

		string stage = "-"+md.runstages;
		stringstream xproc;
		xproc<<nthproc;
		string stagex=stage+xproc.str();
		...
		if (md.outRegn) {
			...
			runcht.regnouter.init(md.outputdir, stagex, maxoutyrs);	 //set netcdf files for output, note NOT output from "eq" run
		 }

 		// 3)for restart.nc outputs
		runcht.resouter.init(md.outputdir, stagex);       //define netcdf file for restart output
...
void Runner::setupData(){
		...
		rungrd.grid.setRegionData(&runreg.region.rd);
 		...
 		runcht.cht.setProcessData(&chtfd);  //
...
void Runner::setupIDs(const int &nprocs){
	...
	//parallel process: start index of cht list, and numbers of chts for each individual process
	int totchts = runchtlist.size();
	int pchts   = floor(totchts/nprocs); // evenly divide chorts over processes
	int reschts = totchts- pchts*nprocs; // reminder after evenly division
	int chtindx0 = 0;     // the starting index of chts
	for (int pid=0; pid<nprocs; pid++) {
		int chtno = pchts;    // the no. of chts
		if (pid<reschts) chtno=pchts+1;  // reminder of cohorts put on the first 'reschts' processes by 1 more than average
		pchtno.push_back(chtno);

		if (pid>0) chtindx0+=pchtno.at(pid-1);     // the starting index of chts
		pchtindx0.push_back(chtindx0);
	}
...
void Runner::runmode2(const int &mypid){
	...
	//loop through cohort in 'runchtlist', which divided over processes
	unsigned int chtindx0=pchtindx0.at(mypid); //starting chts location
	unsigned int chtindx1=pchtindx0.at(mypid)+pchtno.at(mypid); //ending chts location
	for (jj=chtindx0; jj<chtindx1; jj++){
			...
			cout<<"cohort: "<<chtid<<" - running! @ process: "<<mypid<<"\n";
			runcht.run_cohortly();

			//'restart.nc' always output at the end of run-time
			runcht.resouter.outputVariables(runcht.cohortcount);

			runcht.cohortcount++;

	}// end of cohort loop on 1 pid

};

void Runner::runmode3(const int &nthproc){
