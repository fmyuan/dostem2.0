...
class Runner {
	public:
		...
		void initInput(const string &controlfile, const string &runmode, const int &nthproc); /* general initialization */
		void initOutput(const int &nthproc);
		void setupData();
		void setupIDs(const int &nprocs);

		/* three settings for running TEM */
    	void runmode1();  /* one site run-mode, used for stand-alone TEM for any purpose */
    	void runmode2(const int &nthproc);  /* multi-site (regional) run-mode 1, i.e., time series */
    	void runmode3(const int &nthproc);  /* multi-site (regional) run-mode 2, i.e., spatially */
    	...
    	vector<int> pchtno; //numbers of cht for individual parallel process
    	vector<int> pchtindx0;  //start cht location for individual parallel process
