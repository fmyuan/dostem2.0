...
class CohortData{
  	public:
	 	...
	 	int day;
  		...
  		bool hasnonvascular;  //if exists non-vascular within the vegetation community
  		double fnonvascular;  // if exist non-vascular, what is the fraction
		...
		float vapo[MAX_ATM_DRV_YR*12];
  		...
  		deque <double> prveetmxque;   //the last 10 years eet/ppt for long-lasting effect of drought on GPP, through f(phenology)
		deque <double> prvunnormleafmxque;      // deque to store 'unnormleafmx' of at-most previous 10 years
		deque <double> prvgrowingttimeque;      // deque to store 'thermal time (degree-day)' of at-most previous 10 years
		deque <double> toptque;                 // a deque-array to store previous 10 year 'topt'
