...
void EnvOutputer::init(string & dirfile){
	...
	// atm variables
  	...
  	vpV  = ncfileenv->add_var("VAPO", ncDouble, timeD);
   	...
   	vegwaterV= ncfileenv->add_var("CANOPYRAIN", ncDouble, timeD);
   	vegsnowV= ncfileenv->add_var("CANOPYSNOW", ncDouble, timeD);
   	vegrcV= ncfileenv->add_var("CANOPYRC", ncDouble, timeD);
   	vegccV= ncfileenv->add_var("CANOPYCC", ncDouble, timeD);
   	vegbtranV= ncfileenv->add_var("CANOPYBTRAN", ncDouble, timeD);
   	vegm_ppfdV= ncfileenv->add_var("CANOPYM_PPFD", ncDouble, timeD);
   	vegm_vpdV= ncfileenv->add_var("CANOPYM_VPD", ncDouble, timeD);

   	vegswreflV= ncfileenv->add_var("CANOPYSWREFL", ncDouble, timeD);
   	vegswthflV= ncfileenv->add_var("CANOPYSWTHFL", ncDouble, timeD);

   	vegevapV= ncfileenv->add_var("CANOPYEVAP", ncDouble, timeD);
   	vegtranV= ncfileenv->add_var("CANOPYTRAN", ncDouble, timeD);
   	vegevap_pV= ncfileenv->add_var("CANOPYPEVAP", ncDouble, timeD);
   	vegtran_pV= ncfileenv->add_var("CANOPYPTRAN", ncDouble, timeD);
   	vegsublimV= ncfileenv->add_var("CANOPYSUBLIM", ncDouble, timeD);

   	vegrdripV= ncfileenv->add_var("CANOPYRDRIP", ncDouble, timeD);
   	vegrthflV= ncfileenv->add_var("CANOPYRTHFL", ncDouble, timeD);
   	vegsdripV= ncfileenv->add_var("CANOPYSDRIP", ncDouble, timeD);
   	vegsthflV= ncfileenv->add_var("CANOPYSTHFL", ncDouble, timeD);
...
void EnvOutputer::outputCohortEnvVars_dly(EnvDataDly * envod, const int&iy, const int&im, const int &id, const int & tstepcnt){
 	NcError err(NcError::verbose_nonfatal);
 		...
 		vegwaterV->set_cur(tstepcnt);
 		vegwaterV->put(&envod->d_vegs.rwater, 1);

 		vegsnowV->set_cur(tstepcnt);
 		vegsnowV->put(&envod->d_vegs.snow, 1);

 		vegrcV->set_cur(tstepcnt);
 		vegrcV->put(&envod->d_vegd.rc,1);

 		vegccV->set_cur(tstepcnt);
 		vegccV->put(&envod->d_vegd.cc, 1);

 		vegbtranV->set_cur(tstepcnt);
   		vegbtranV->put(&envod->d_vegd.btran, 1);

   		vegm_ppfdV->set_cur(tstepcnt);
   		vegm_ppfdV->put(&envod->d_vegd.m_ppfd, 1);

   		vegm_vpdV->set_cur(tstepcnt);
   		vegm_vpdV->put(&envod->d_vegd.m_vpd, 1);

 		vegswreflV->set_cur(tstepcnt);
   		vegswreflV->put(&envod->d_v2a.swrefl, 1);

   		vegswthflV->set_cur(tstepcnt);
   		vegswthflV->put(&envod->d_v2g.swthfl, 1);

 		vegevapV->set_cur(tstepcnt);
   		vegevapV->put(&envod->d_v2a.evap, 1);

   		vegtranV->set_cur(tstepcnt);
   		vegtranV->put(&envod->d_v2a.tran, 1);

   		vegevap_pV->set_cur(tstepcnt);
   		vegevap_pV->put(&envod->d_v2a.evap_pet, 1);

   		vegtran_pV->set_cur(tstepcnt);
   		vegtran_pV->put(&envod->d_v2a.tran_pet, 1);

   		vegsublimV->set_cur(tstepcnt);
   		vegsublimV->put(&envod->d_v2a.sublim, 1);

 		vegrdripV->set_cur(tstepcnt);
   		vegrdripV->put(&envod->d_v2g.rdrip, 1);

   		vegrthflV->set_cur(tstepcnt);
   		vegrthflV->put(&envod->d_v2g.rthfl, 1);

   		vegsdripV->set_cur(tstepcnt);
   		vegsdripV->put(&envod->d_v2g.sdrip, 1);

   		vegsthflV->set_cur(tstepcnt);
   		vegsthflV->put(&envod->d_v2g.sthfl, 1);
...
void EnvOutputer::outputCohortEnvVars_mly(snwstate_dim *m_snow, EnvData* envod, const int&iy, const int&im, const int & tstepcnt){
 	NcError err(NcError::verbose_nonfatal);
 		...
 		//canopy-env variables for ipft
 		vegwaterV->set_cur(tstepcnt);
   		vegwaterV->put(&envod->m_vegs.rwater, 1);

 		vegsnowV->set_cur(tstepcnt);
   		vegsnowV->put(&envod->m_vegs.snow, 1);

 		vegrcV->set_cur(tstepcnt);
 		vegrcV->put(&envod->m_vegd.rc, 1);

 		vegccV->set_cur(tstepcnt);
 		vegccV->put(&envod->m_vegd.cc, 1);

 		vegbtranV->set_cur(tstepcnt);
   		vegbtranV->put(&envod->m_vegd.btran, 1);

   		vegm_ppfdV->set_cur(tstepcnt);
   		vegm_ppfdV->put(&envod->m_vegd.m_ppfd, 1);

   		vegm_vpdV->set_cur(tstepcnt);
   		vegm_vpdV->put(&envod->m_vegd.m_vpd, 1);

 		vegswreflV->set_cur(tstepcnt);
   		vegswreflV->put(&envod->m_v2a.swrefl, 1);

   		vegswthflV->set_cur(tstepcnt);
   		vegswthflV->put(&envod->m_v2g.swthfl, 1);

 		vegevapV->set_cur(tstepcnt);
   		vegevapV->put(&envod->m_v2a.evap, 1);

   		vegtranV->set_cur(tstepcnt);
   		vegtranV->put(&envod->m_v2a.tran, 1);

   		vegevap_pV->set_cur(tstepcnt);
   		vegevap_pV->put(&envod->m_v2a.evap_pet, 1);

   		vegtran_pV->set_cur(tstepcnt);
   		vegtran_pV->put(&envod->m_v2a.tran_pet, 1);

   		vegsublimV->set_cur(tstepcnt);
   		vegsublimV->put(&envod->m_v2a.sublim, 1);

 		vegrdripV->set_cur(tstepcnt);
   		vegrdripV->put(&envod->m_v2g.rdrip, 1);

   		vegrthflV->set_cur(tstepcnt);
   		vegrthflV->put(&envod->m_v2g.rthfl, 1);

   		vegsdripV->set_cur(tstepcnt);
   		vegsdripV->put(&envod->m_v2g.sdrip, 1);

   		vegsthflV->set_cur(tstepcnt);
   		vegsthflV->put(&envod->m_v2g.sthfl, 1);
...
void EnvOutputer::outputCohortEnvVars_yly(snwstate_dim* y_snow, EnvData *envod, const int&iy, const int & tstepcnt){
 	NcError err(NcError::verbose_nonfatal);
 		...
 		//canopy-env variables for ipft
 		vegwaterV->set_cur(tstepcnt);
 		vegwaterV->put(&envod->y_vegs.rwater, 1);

 		vegsnowV->set_cur(tstepcnt);
 		vegsnowV->put(&envod->y_vegs.snow, 1);

 		vegrcV->set_cur(tstepcnt);
 		vegrcV->put(&envod->y_vegd.rc, 1);

 		vegccV->set_cur(tstepcnt);
 		vegccV->put(&envod->y_vegd.cc, 1);

 		vegbtranV->set_cur(tstepcnt);
   		vegbtranV->put(&envod->y_vegd.btran, 1);

   		vegm_ppfdV->set_cur(tstepcnt);
   		vegm_ppfdV->put(&envod->y_vegd.m_ppfd, 1);

   		vegm_vpdV->set_cur(tstepcnt);
   		vegm_vpdV->put(&envod->y_vegd.m_vpd, 1);

 		vegswreflV->set_cur(tstepcnt);
   		vegswreflV->put(&envod->y_v2a.swrefl, 1);

   		vegswthflV->set_cur(tstepcnt);
   		vegswthflV->put(&envod->y_v2g.swthfl, 1);

 		vegevapV->set_cur(tstepcnt);
   		vegevapV->put(&envod->y_v2a.evap, 1);

   		vegtranV->set_cur(tstepcnt);
   		vegtranV->put(&envod->y_v2a.tran, 1);

   		vegevap_pV->set_cur(tstepcnt);
   		vegevap_pV->put(&envod->y_v2a.evap_pet, 1);

   		vegtran_pV->set_cur(tstepcnt);
   		vegtran_pV->put(&envod->y_v2a.tran_pet, 1);

   		vegsublimV->set_cur(tstepcnt);
   		vegsublimV->put(&envod->y_v2a.sublim, 1);

 		vegrdripV->set_cur(tstepcnt);
   		vegrdripV->put(&envod->y_v2g.rdrip, 1);

   		vegrthflV->set_cur(tstepcnt);
   		vegrthflV->put(&envod->y_v2g.rthfl, 1);

   		vegsdripV->set_cur(tstepcnt);
   		vegsdripV->put(&envod->y_v2g.sdrip, 1);

   		vegsthflV->set_cur(tstepcnt);
   		vegsthflV->put(&envod->y_v2g.sthfl, 1);
