...
void RestartOutputer::init(string& outputdir,string& stage){
	...
	ifwoodyV     =restartFile->add_var("IFWOODY", ncInt, chtD);
	ifdeciwoodyV =restartFile->add_var("IFDECIWOODY", ncInt, chtD);
	ifperenialV  =restartFile->add_var("IFPERENIAL", ncInt, chtD);
	nonvascularV =restartFile->add_var("NONVASCULAR", ncInt, chtD);
	fnonvascularV =restartFile->add_var("FNONVASCULAR", ncInt, chtD);

	vegageV  =restartFile->add_var("VEGAGE", ncInt, chtD);
	vegcovV  =restartFile->add_var("VEGCOV", ncDouble, chtD);
	laiV     =restartFile->add_var("LAI", ncDouble, chtD);
	rootfracV=restartFile->add_var("ROOTFRAC", ncDouble, chtD, rootlayerD);
    vegwaterV=restartFile->add_var("VEGWATER", ncDouble, chtD);
    vegsnowV =restartFile->add_var("VEGSNOW", ncDouble, chtD);

    vegcV   =restartFile->add_var("VEGC", ncDouble, chtD, pftpartD);
    strnV   =restartFile->add_var("STRN", ncDouble, chtD, pftpartD);
    labnV   =restartFile->add_var("LABN", ncDouble, chtD);
    deadcV  =restartFile->add_var("DEADC", ncDouble, chtD);
    deadnV  =restartFile->add_var("DEADN", ncDouble, chtD);

 	toptV         =restartFile->add_var("TOPT", ncDouble, chtD);
	eetmxV        =restartFile->add_var("EETMX", ncDouble, chtD);
 	growingttimeV =restartFile->add_var("GROWINGTTIME", ncDouble, chtD);
	unnormleafmxV =restartFile->add_var("UNNORMLEAFMX", ncDouble, chtD);
	foliagemxV =restartFile->add_var("FOLIAGEMX", ncDouble, chtD);

	toptAV         =restartFile->add_var("TOPTA", ncDouble, chtD, prvyearD);
	eetmxAV        =restartFile->add_var("EETMXA", ncDouble, chtD, prvyearD);
 	growingttimeAV =restartFile->add_var("GROWINGTTIMEA", ncDouble, chtD, prvyearD);
	unnormleafmxAV =restartFile->add_var("UNNORMLEAFMXA", ncDouble, chtD, prvyearD);
...
void RestartOutputer::outputVariables(const int & chtcount){
	...
	ifwoodyV->put_rec(&resod->ifwoody, chtcount);
	ifdeciwoodyV->put_rec(&resod->ifdeciwoody, chtcount);
	ifperenialV->put_rec(&resod->ifperenial, chtcount);
	nonvascularV->put_rec(&resod->nonvascular, chtcount);
	fnonvascularV->put_rec(&resod->fnonvascular, chtcount);
	vegageV->put_rec(&resod->vegage, chtcount);
	vegcovV->put_rec(&resod->vegcov, chtcount);
	laiV->put_rec(&resod->lai, chtcount);
	rootfracV->put_rec(&resod->rootfrac[0], chtcount);

	vegwaterV->put_rec(&resod->vegwater, chtcount);
	vegsnowV->put_rec(&resod->vegsnow, chtcount);

	vegcV->put_rec(&resod->vegc[0], chtcount);
	strnV->put_rec(&resod->strn[0], chtcount);
	labnV->put_rec(&resod->labn, chtcount);
	deadcV->put_rec(&resod->deadc, chtcount);
	deadnV->put_rec(&resod->deadn, chtcount);

    toptV->put_rec(&resod->topt, chtcount);
	eetmxV->put_rec(&resod->eetmx, chtcount);
	growingttimeV->put_rec(&resod->growingttime, chtcount);
	unnormleafmxV->put_rec(&resod->unnormleafmx, chtcount);
	foliagemxV->put_rec(&resod->foliagemx, chtcount);

	toptAV->put_rec(&resod->toptA[0], chtcount);
	eetmxAV->put_rec(&resod->eetmxA[0], chtcount);
	growingttimeAV->put_rec(&resod->growingttimeA[0], chtcount);
