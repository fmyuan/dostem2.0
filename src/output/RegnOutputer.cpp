	...
	regnod->yrsdist = MISSING_I;
  	for(int im=0; im<12; im++){
		...
		regnod->month[im]= MISSING_I;
   			...
   			regnod->growstart[im] = MISSING_I;
			regnod->growend[im]   = MISSING_I;

			regnod->vegcov[im]= MISSING_D;
			regnod->vegage[im] = MISSING_D;
			regnod->lai[im]  = MISSING_D;
			regnod->vegc[im] = MISSING_D;
			regnod->leafc[im]= MISSING_D;
			regnod->stemc[im]= MISSING_D;
			regnod->rootc[im]= MISSING_D;
			regnod->vegn[im] = MISSING_D;
			regnod->labn[im] = MISSING_D;
			regnod->leafn[im]= MISSING_D;
			regnod->stemn[im]= MISSING_D;
			regnod->rootn[im]= MISSING_D;

			regnod->gpp[im] = MISSING_D;
			regnod->npp[im] = MISSING_D;
			regnod->ltrfalc[im]= MISSING_D;
			regnod->ltrfaln[im]= MISSING_D;

			regnod->nuptake[im]= MISSING_D;
...
/////////////////* YEARLY outputs *//////////////////////////////////////////////
	//
   	if (regnod->outvarlist[I_growstart]==1)
   		growstartV = rFile->add_var("GROWSTART", ncInt, chtD, yearD);
   	if (regnod->outvarlist[I_growend]==1)
   		growendV   = rFile->add_var("GROWEND", ncInt, chtD, yearD);

   	if (regnod->outvarlist[I_vegcov]==1)
   		vegcovV   = rFile->add_var("VEGCOV", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_vegage]==1)
   		vegageV    = rFile->add_var("VEGAGE", ncInt, chtD, yearD);

   	if (regnod->outvarlist[I_lai]==1)
   		laiV       = rFile->add_var("LAI", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_vegc]==1)
   		vegcV      = rFile->add_var("VEGC", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_leafc]==1)
   		leafcV     = rFile->add_var("LEAFC", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_stemc]==1)
   		stemcV     = rFile->add_var("STEMC", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_rootc]==1)
   		rootcV     = rFile->add_var("ROOTC", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_vegn]==1)
   		vegnV      = rFile->add_var("VEGN", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_labn]==1)
   		labnV      = rFile->add_var("LABN", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_leafn]==1)
   		leafnV     = rFile->add_var("LEAFN", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_stemn]==1)
   		stemnV     = rFile->add_var("STEMN", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_rootn]==1)
   		rootnV     = rFile->add_var("ROOTN", ncDouble, chtD, yearD);

   	if (regnod->outvarlist[I_gpp]==1)
   		gppV       = rFile->add_var("GPP", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_npp]==1)
   		nppV       = rFile->add_var("NPP", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_ltrfalc]==1)
        ltrfalcV   = rFile->add_var("LTRFALC", ncDouble, chtD, yearD);
   	if (regnod->outvarlist[I_ltrfaln]==1)
        ltrfalnV   = rFile->add_var("LTRFALN", ncDouble, chtD, yearD);

   	if (regnod->outvarlist[I_nuptake]==1)
        nuptakeV   = rFile->add_var("NUPTAKE", ncDouble, chtD, yearD);
...
/////////////////* MONTHLY outputs *//////////////////////////////////////////////
	...
	//
   	if (regnod->outvarlist[I_growstart]==2)
   		growstartV = rFile->add_var("GROWSTART", ncInt, chtD, yrmonD);
   	if (regnod->outvarlist[I_growend]==2)
   		growendV   = rFile->add_var("GROWEND", ncInt, chtD, yrmonD);

   	if (regnod->outvarlist[I_vegcov]==2)
   		vegcovV   = rFile->add_var("VEGFRAC", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_vegage]==2)
   		vegageV    = rFile->add_var("VEGAGE", ncInt, chtD, yrmonD);

   	if (regnod->outvarlist[I_lai]==2)
   		laiV       = rFile->add_var("LAI", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_vegc]==2)
   		vegcV      = rFile->add_var("VEGC", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_leafc]==2)
   		leafcV     = rFile->add_var("LEAFC", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_stemc]==2)
   		stemcV     = rFile->add_var("STEMC", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_rootc]==2)
   		rootcV     = rFile->add_var("ROOTC", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_vegn]==2)
   		vegnV      = rFile->add_var("VEGN", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_labn]==2)
   		labnV      = rFile->add_var("LABN", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_leafn]==2)
   		leafnV     = rFile->add_var("LEAFN", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_stemn]==2)
   		stemnV     = rFile->add_var("STEMN", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_rootn]==2)
   		rootnV     = rFile->add_var("ROOTN", ncDouble, chtD, yrmonD);

   	if (regnod->outvarlist[I_gpp]==2)
   		gppV       = rFile->add_var("GPP", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_npp]==2)
   		nppV       = rFile->add_var("NPP", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_ltrfalc]==2)
        ltrfalcV   = rFile->add_var("LTRFALC", ncDouble, chtD, yrmonD);
   	if (regnod->outvarlist[I_ltrfaln]==2)
        ltrfalnV   = rFile->add_var("LTRFALN", ncDouble, chtD, yrmonD);

   	if (regnod->outvarlist[I_nuptake]==2)
        nuptakeV   = rFile->add_var("NUPTAKE", ncDouble, chtD, yrmonD);
...
/////// YEARLY OUTPUT /////////////////////////////////////////////////////////////////
   	if (regnod->outvarlist[I_growstart]==1){
		...
		growstartV->put(&regnod->growstart[0], 1);
   	}

   	if (regnod->outvarlist[I_growend]==1){
   	   	growendV->set_cur(chtcount, yrind);
		growendV->put(&regnod->growend[0], 1);
   	}

   	if (regnod->outvarlist[I_vegcov]==1){
   	   	vegcovV->set_cur(chtcount, yrind);
		vegcovV->put(&regnod->vegcov[0], 1);
   	}

   	if (regnod->outvarlist[I_vegage]==1){
   	   	vegageV->set_cur(chtcount, yrind);
		vegageV->put(&regnod->vegage[0], 1);
   	}

   	if (regnod->outvarlist[I_lai]==1){
   	   	laiV->set_cur(chtcount, yrind);
		laiV->put(&regnod->lai[0], 1);
   	}

   	if (regnod->outvarlist[I_vegc]==1){
   	   	vegcV->set_cur(chtcount, yrind);
		vegcV->put(&regnod->vegc[0], 1);
   	}

   	if (regnod->outvarlist[I_leafc]==1){
   	   	leafcV->set_cur(chtcount, yrind);
		leafcV->put(&regnod->leafc[0], 1);
   	}

   	if (regnod->outvarlist[I_stemc]==1){
   	   	stemcV->set_cur(chtcount, yrind);
		stemcV->put(&regnod->stemc[0], 1);
   	}

   	if (regnod->outvarlist[I_rootc]==1){
   	   	rootcV->set_cur(chtcount, yrind);
		rootcV->put(&regnod->rootc[0], 1);
   	}

   	if (regnod->outvarlist[I_vegn]==1){
   	   	vegnV->set_cur(chtcount, yrind);
		vegnV->put(&regnod->vegn[0], 1);
   	}

   	if (regnod->outvarlist[I_labn]==1){
   	   	labnV->set_cur(chtcount, yrind);
		labnV->put(&regnod->labn[0], 1);
   	}

   	if (regnod->outvarlist[I_leafn]==1){
   	   	leafnV->set_cur(chtcount, yrind);
		leafnV->put(&regnod->leafn[0], 1);
   	}

   	if (regnod->outvarlist[I_stemn]==1){
   	   	stemnV->set_cur(chtcount, yrind);
		stemnV->put(&regnod->stemn[0], 1);
   	}

   	if (regnod->outvarlist[I_rootn]==1){
   	   	rootnV->set_cur(chtcount, yrind);
		rootnV->put(&regnod->rootn[0], 1);
   	}

   	if (regnod->outvarlist[I_gpp]==1){
   	   	gppV->set_cur(chtcount, yrind);
		gppV->put(&regnod->gpp[0], 1);
   	}

   	if (regnod->outvarlist[I_npp]==1){
   	   	nppV->set_cur(chtcount, yrind);
		nppV->put(&regnod->npp[0], 1);
   	}

   	if (regnod->outvarlist[I_ltrfalc]==1){
   	   	ltrfalcV->set_cur(chtcount, yrind);
   	   	ltrfalcV->put(&regnod->ltrfalc[0], 1);
   	}

   	if (regnod->outvarlist[I_ltrfaln]==1){
   	   	ltrfalnV->set_cur(chtcount, yrind);
   	   	ltrfalnV->put(&regnod->ltrfaln[0], 1);
   	}

   	if (regnod->outvarlist[I_nuptake]==1){
   	   	nuptakeV->set_cur(chtcount, yrind);
   	   	nuptakeV->put(&regnod->nuptake[0], 1);
   	}
