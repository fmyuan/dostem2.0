...
void WildFire::initializeParameter(){
	...
	for (int i=0; i<NUM_FSEVR; i++){
		firpar.fvcomb[i] = chtlu->fvcombust[i];
		firpar.fvdead[i] = chtlu->fvslash[i];
...
// for soil part and root burning
 	...
 	double r_burn2bg_cn; //ratio of dead veg. after burning
 	r_burn2bg_cn = 0.; //used for vegetation below-ground (root) loss, and calculated below

 	for (int il =0; il <cd->m_soil.numsl; il++){
	  	if(cd->m_soil.type[il]<=2){ //dead moss is 0, shlw peat is 1 and deep org is 2

	  		// dead moss layer (burn all dead moss biomass)
	  		if (bd->m_sois.dmossc>0.) {
	  			burnedsolc +=bd->m_sois.dmossc;
	  			burnedsoln +=bd->m_sois.dmossn;
	  		...
	  		double ilsolc =  bd->m_sois.rawc[il]+bd->m_sois.soma[il]
	  		       +bd->m_sois.sompr[il]+bd->m_sois.somcr[il];
	  		double ilsoln =  bd->m_sois.orgn[il]+bd->m_sois.avln[il];
	  		...
	  		if(totbotdepth<=burndepth){ //remove all the orgc/n in this layer
  		    	...
  		    	if (cd->m_veg.vegcov>0.){
  		    		r_burn2bg_cn += cd->m_soil.frootfrac[il];
	  	  		...
	  	  		if(partleft<cd->m_soil.dz[il]){
	  	  			...
	  	  			bd->m_sois.rawc[il] *= partleft/cd->m_soil.dz[il];
	  	  			bd->m_sois.soma[il] *= partleft/cd->m_soil.dz[il];
	  	  			bd->m_sois.sompr[il] *= partleft/cd->m_soil.dz[il];
	  	  			bd->m_sois.somcr[il] *= partleft/cd->m_soil.dz[il];

	  	  			bd->m_sois.orgn[il] *= partleft/cd->m_soil.dz[il];
	  	  			bd->m_sois.avln[il] *= partleft/cd->m_soil.dz[il];

	  		    	if (cd->m_veg.vegcov>0.){
		  		    		r_burn2bg_cn += (1-partleft/cd->m_soil.dz[il])* cd->m_soil.frootfrac[il];
		  		    		cd->m_soil.frootfrac[il] *=partleft/cd->m_soil.dz[il];
		  	  		}
    ...
    // needs to re-do the soil rootfrac for each pft which was modified above (in burn soil layer)
 		...
 		for (int il =0; il <cd->m_soil.numsl; il++){
 			rootfracsum += cd->m_soil.frootfrac[il];
 		}
 		for (int il =0; il <cd->m_soil.numsl; il++){
 			cd->m_soil.frootfrac[il] /=rootfracsum;
 		}

     // all woody debris will burn out
	double wdebrisc = bd->m_sois.wdebrisc; //
	double wdebrisn = bd->m_sois.wdebrisn; //
	bd->m_sois.wdebrisc = 0.;
	bd->m_sois.wdebrisn = 0.;
...
//////////////////////VEG burning and mortality ///////////////////////////////////////////////////////////////
	...
	double dead_bg_vegn = 0.;
   	...
   	if (cd->m_veg.vegcov>0.){
    		...
    		getBurnAbgVegetation();

    		//root death ratio: must be called after both above-ground and below-ground burning
    		double r_dead2bg_cn = 1.0-r_burn2bg_cn- r_live_cn;   //r_live_cn is same for both above-ground and below-ground

    		// dead veg c/n
    		comb_deadc += bd->m_vegs.deadc*cd->m_veg.vegcov;         //assuming all previous deadc burned
    		comb_deadn += bd->m_vegs.deadn*cd->m_veg.vegcov;         //assuming all previous deadn burned
    		bd->m_vegs.deadc = 0.;
    		bd->m_vegs.deadn = 0.;

    		// above-ground veg. burning/death during fire
    		comb_vegc += bd->m_vegs.c[I_leaf]*r_burn2ag_cn*cd->m_veg.vegcov;     // when summing, needs adjusting by 'vegcov'
    		bd->m_vegs.deadc = bd->m_vegs.c[I_leaf]*r_dead2ag_cn;   // we define deadc/n as the not-falling veg (or binding with living veg) during fire,
		                                                        // which then is the source of ground debris (this is for woody plants only, others could be set deadc/n to zero)
    		bd->m_vegs.c[I_leaf] *= (1.0-r_burn2ag_cn-r_dead2ag_cn);

    		comb_vegc += bd->m_vegs.c[I_stem]*r_burn2ag_cn*cd->m_veg.vegcov;
    		bd->m_vegs.deadc += bd->m_vegs.c[I_stem]*r_dead2ag_cn;
    		bd->m_vegs.c[I_stem] *= (1.0-r_burn2ag_cn-r_dead2ag_cn);

    		comb_vegn += bd->m_vegs.strn[I_leaf]*r_burn2ag_cn*cd->m_veg.vegcov;
    		bd->m_vegs.deadn += bd->m_vegs.strn[I_leaf]*r_dead2ag_cn;
    		bd->m_vegs.strn[I_leaf] *= (1.0-r_burn2ag_cn-r_dead2ag_cn);

    		comb_vegn += bd->m_vegs.strn[I_stem]*r_burn2ag_cn;
    		bd->m_vegs.deadn += bd->m_vegs.strn[I_stem]*r_dead2ag_cn;
    		bd->m_vegs.strn[I_stem] *= (1.0-r_burn2ag_cn-r_dead2ag_cn);

    		// below-ground veg. (root) burning/death during fire
    		comb_vegc += bd->m_vegs.c[I_root]*r_burn2bg_cn*cd->m_veg.vegcov;
    		double deadc_tmp = bd->m_vegs.c[I_root]*r_dead2bg_cn*cd->m_veg.vegcov;
    		for (int il =0; il <cd->m_soil.numsl; il++){
	    	// for the dead below-ground C/N caused by fire, they are put into original layer
    			if (cd->m_soil.frootfrac[il]>0.)
	    		bd->m_sois.somcr[il] += deadc_tmp*cd->m_soil.frootfrac[il];   //'rootfrac' must be updated above
    		}
    		dead_bg_vegc +=deadc_tmp;

    		bd->m_vegs.c[I_root] *= (1.0-r_burn2bg_cn-r_dead2bg_cn);

    		comb_vegn += bd->m_vegs.strn[I_root]*r_burn2bg_cn*cd->m_veg.vegcov;
    		double deadn_tmp = bd->m_vegs.strn[I_root]*r_dead2bg_cn*cd->m_veg.vegcov;   // this is needed below
    		for (int il =0; il <cd->m_soil.numsl; il++){
    			// for the dead below-ground C/N caused by fire, they are put into original layer
    			if (cd->m_soil.frootfrac[il]>0.)
	    		bd->m_sois.somcr[il] += deadn_tmp*cd->m_soil.frootfrac[il];   //'rootfrac' must be updated above
    		}
    		dead_bg_vegn +=deadn_tmp;
    		bd->m_vegs.strn[I_root] *= (1.0-r_burn2bg_cn-r_dead2bg_cn);

    		// one more veg N pool (labile N)
    		comb_vegn += bd->m_vegs.labn*(1.-r_live_cn)*cd->m_veg.vegcov;  //assuming all labn emitted, leaving none into deadn
    		bd->m_vegs.labn *= r_live_cn;

    		// finally, we have:
    		bd->m_vegs.call = bd->m_vegs.c[I_leaf]+bd->m_vegs.c[I_stem]+bd->m_vegs.c[I_root];
    		bd->m_vegs.nall = bd->m_vegs.strn[I_leaf]+bd->m_vegs.strn[I_stem]+bd->m_vegs.strn[I_root]+bd->m_vegs.labn;
...
/////////////////////////////////////////////////////////////////////////////////
   ...
   // Note - this 'retained C' could be used as char-coal, if need to do so. Then define the 'r_retain_c' in the model shall be workable
    for (int il =0; il <cd->m_soil.numsl; il++){
	    double tsomc = bd->m_sois.rawc[il]+bd->m_sois.soma[il]+bd->m_sois.sompr[il]+bd->m_sois.somcr[il];
  		if(tsomc > 0. || il==cd->m_soil.numsl-1){
  		// this may possibly put retac/n in the first mineral soil
  			bd->m_sois.somcr[il] += reta_vegc + reta_solc;
  			bd->m_sois.orgn[il]  += reta_vegn + reta_soln;
...
void WildFire::getBurnAbgVegetation() {
	...
	if (oneseverity==0) {
			r_burn2ag_cn = firpar.fvcomb[0];
			r_dead2ag_cn = firpar.fvdead[0];
	}else if (oneseverity==1) {
			r_burn2ag_cn = firpar.fvcomb[1];
			r_dead2ag_cn = firpar.fvdead[1];
	}else if (oneseverity==2) {
			r_burn2ag_cn = firpar.fvcomb[2];
			r_dead2ag_cn = firpar.fvdead[2];
	}else if (oneseverity==3) {
			r_burn2ag_cn = firpar.fvcomb[3];
			r_dead2ag_cn = firpar.fvdead[3];
	}else if (oneseverity==4) {
			r_burn2ag_cn = firpar.fvcomb[4];
			r_dead2ag_cn = firpar.fvdead[4];
	}
