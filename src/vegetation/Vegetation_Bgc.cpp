...
void Vegetation_Bgc::initializeParameter(){
  	...
  	bgcpar.kc      = chtlu->kc;
  	bgcpar.ki      = chtlu->ki;
	bgcpar.tmin    = chtlu->tmin;
	bgcpar.toptmin = chtlu->toptmin;
	bgcpar.toptmax = chtlu->toptmax;
	bgcpar.tmax    = chtlu->tmax;

  	bgcpar.raq10a0 = chtlu->raq10a0;
  	bgcpar.raq10a1 = chtlu->raq10a1;
  	bgcpar.raq10a2 = chtlu->raq10a2;
  	bgcpar.raq10a3 = chtlu->raq10a3;
  
  	bgcpar.knuptake = chtlu->knuptake;
  
  	for (int i=0; i<NUM_PFT_PART; i++) {
  		bgcpar.cpart[i]   = chtlu->cpart[i];
  	}

  	bgcpar.c2na = chtlu->c2na;
  	for (int i=0; i<NUM_PFT_PART; i++) {
  		bgcpar.c2nb[i]   = chtlu->c2nb[i];
  		bgcpar.c2neven[i] = chtlu->initc2neven[i];
  	  	bgcpar.c2nmin[i] = chtlu->c2nmin[i];
  	}
  	bgcpar.labncon = chtlu->labncon;

  	// Calibrated parameters for vegetation BGC
  	calpar.cmax = chtlu->cmax;
  	calpar.nmax = chtlu->nmax;
  	for (int i=0; i<NUM_PFT_PART; i++) {
  		calpar.cfall[i] = chtlu->cfall[i];
  		calpar.nfall[i] = chtlu->nfall[i];
  	}
  	calpar.kra = chtlu->kra;
  	for (int i=0; i<NUM_PFT_PART; i++) {
  		calpar.krb[i] = chtlu->krb[i];
  	}
  	calpar.frg = chtlu->frg;
...
void Vegetation_Bgc::initializeState(){
	...
	double totvegn = 0.;
	for (int i=0; i<NUM_PFT_PART; i++){
		bd->m_vegs.c[i]    = chtlu->initvegc[i];
		bd->m_vegs.strn[i] = chtlu->initvegn[i]*0.95;

		totvegn += chtlu->initvegn[i];
	}
	bd->m_vegs.labn  = totvegn*0.05;

	bd->m_vegs.deadc = chtlu->initdeadc;
	bd->m_vegs.deadn = chtlu->initdeadn;
...
void Vegetation_Bgc::initializeState5restart(RestartData *resin){
	...
	for (int i=0; i<NUM_PFT_PART; i++){
	  bd->m_vegs.c[i]    = resin->vegc[i];
	  bd->m_vegs.strn[i] = resin->strn[i];
	}
	bd->m_vegs.labn = resin->labn;

	bd->m_vegs.deadc = resin->deadc;
	bd->m_vegs.deadn = resin->deadn;
...
void Vegetation_Bgc::prepareIntegration(const bool &nfeedback){
	...
	double prvtopt = 0.;
	deque<double> toptdeque = cd->toptque;
	...
	dleafc = cd->m_vegd.maxleafc*cd->m_vegd.fleaf;  //yearly max. leaf C, adjusted by seasonal foliage growth index 'fleaf'
	...
	deque<double> ttimedeque = cd->prvgrowingttimeque;
	...
	for (int i=0; i<NUM_PFT_PART; i++){
		// assuming calibrated 'calpar.cfall and calpar.nfall' is the max. monthly fraction, we need to modify them for specific PFT types
	...
	if (cd->m_vegd.growingttime>0. && prvttime>0.) {
		fltrfall = fmin(1., cd->m_vegd.growingttime/prvttime);
	}
...
void Vegetation_Bgc::delta(){
	...
	// 1) current EET and previous max. EET controlled
  	double fleaf = cd->m_vegd.fleaf;
  
  	// 2) plant size (biomass C) or age controlled
 	double ffoliage = cd->m_vegd.ffoliage;
...
void Vegetation_Bgc::deltanfeed(){
	...
	if(nfeed){
		...
		//if (cd->m_veg.fnonvascular<=0.50) {
			del_soi2v.innuptake = getNuptake(cd->m_vegd.ffoliage, bd->m_vegd.raq10,
	  			                             bgcpar.knuptake, calpar.nmax);
			del_soi2v.innuptake *= (1.0-cd->m_veg.fnonvascular);
		//} else {
			del_soi2v.innuptake += (cd->m_veg.fnonvascular*calpar.nmax*cd->m_vegd.ffoliage);    //need more mechanism algorithm for non-vascular plants:
		...
		// N requirement less than max.Nuptake+labN (total N potential supply), which may need down-regulated in turn
			...
			double cnmin = 0.; //nppall/fmax(1.e-8, nmin);
...
void Vegetation_Bgc::afterIntegration(){
	...
	for (int i=0; i<NUM_PFT_PART; i++) {
		...
		bd->m_v2soi.ltrfalcall += fmax(0., 1.-cd->m_veg.fnonvascular)*bd->m_v2soi.ltrfalc[i];
		bd->m_v2soi.mossdeathc += fmax(0., cd->m_veg.fnonvascular)*bd->m_v2soi.ltrfalc[i];

		if(nfeed) {
			bd->m_v2soi.ltrfalnall += fmax(0., 1.-cd->m_veg.fnonvascular)*bd->m_v2soi.ltrfaln[i];
			bd->m_v2soi.mossdeathn += fmax(0., cd->m_veg.fnonvascular)*bd->m_v2soi.ltrfaln[i];
...
double Vegetation_Bgc::getNuptake(const double & foliage, const double & raq10,
	...
	for(int il =0; il<cd->m_soil.numsl; il++){
		if (cd->m_soil.frootfrac[il]> 0.) {
			...
			totfrootfrac +=cd->m_soil.frootfrac[il];
		}
    }
	...
	for(int il =0; il<cd->m_soil.numsl; il++){
		fracnuptake[il] = 0.;
		if (cd->m_soil.frootfrac[il]>0. && bd->m_sois.avln[il]>0. && ed->m_sois.liq[il]>0.) {
			fracnuptake[il] = cd->m_soil.frootfrac[il]
