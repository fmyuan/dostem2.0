...
void Vegetation_Env::initializeParameter(){
    envpar.albvisnir = chtlu->albvisnir;
    envpar.er        = chtlu->er;
    envpar.ircoef    = chtlu->ircoef;
    envpar.iscoef    = chtlu->iscoef;

	envpar.gl_bl     = chtlu->gl_bl;
    envpar.gl_c      = chtlu->gl_c;
    
    envpar.vpd_close = chtlu->vpd_close;
    envpar.vpd_open  = chtlu->vpd_open;
    envpar.glmax     = chtlu->glmax;
    envpar.ppfd50    = chtlu->ppfd50;
...
void Vegetation_Env::initializeState(){
	...
	ed->d_vegs.rwater = chtlu->initvegwater;
	ed->d_vegs.snow   = chtlu->initvegsnow;

};

void Vegetation_Env::initializeState5restart(RestartData* resin){
	
	ed->d_vegs.rwater = resin->vegwater;
	ed->d_vegs.snow   = resin->vegsnow;
...
void Vegetation_Env::updateRadiation(){
	...
	//lai/fpc from 'cd'
	double envlai = cd->m_veg.lai;
	double fpc = cd->m_veg.fpc;
