...
struct vegdiag_dim{
    ...
    // phenoloy variables
	double growingttime;     // (current but accumulated) growing thermal time
    double maxleafc;           // max. leaf C limited by plant state itself

    double fleaf;            // (current) normalized (scalar) foliage growth index based on current and previous EET
	double unnormleaf;       // (current) un-normarlized fleaf
	double eetmx;            // (yearly) max. month eet
	double unnormleafmx;     // (yearly max.) un-normarlized fleaf
	double topt;             // (yearly) evolving optimium temperature for temperature-scalar of GPP

	double ffoliage;         // (current)foliage growth index (scalar) based on vegetation C (stand age related)
    double foliagemx;        // this is for 'ffoliage' not growing backward
