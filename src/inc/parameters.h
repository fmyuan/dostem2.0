...
struct vegpar_dim{
	...
	double kfoliage;  // these 2 parameters for non-forest
	double cov;
    double m1;	    // these 4 parameters for forest
    double m2;
    double m3;
    double m4;
...
struct firepar_bgc{
	...
	double fvcomb[NUM_FSEVR];    //fraction of burned total veg. by each PFT, sum of which is 1
	double fvdead[NUM_FSEVR];    //fraction of dead total veg. by each PFT, sum of which is 1
