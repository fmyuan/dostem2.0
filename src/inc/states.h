...
struct vegstate_dim{
    ...
    int ifwoody;            // woody-plant vs non-woody: different functions for 'foliage' equation
    int ifdeciwoody;        // decidous vs evergreen woodland: different 'nfactor' for ground-surface temperture estimation
    int ifperenial;         // perenial plant (1) or not (0): not yet used
    int nonvascular;        // vascular plant (0), sphagnum (1), feathermoss (2), lichen (3)
    double fnonvascular;    // fraction of nonvascular plants
    double vegcov;          // veg. coverage fraction (max. fpc over time, i.e. land coverage fraction (max. value by whole canopy))

    double lai;             // lai
    double fpc;             // foliage percentage coverage (seasonally dynamics), related to LAI
    double frootfrac[MAX_ROT_LAY];    // fine root distribution
...
struct soistate_dim{
	...
	double frootfrac[MAX_SOI_LAY];    //fine root vertical distribution

};

struct soistate_env{

	int frozen[MAX_SOI_LAY];       //totally frozen: 1, totally unfrozen: -1, partially frozen: 0 (daily) or <1~>-1 (for monthly/yearly)
	double frozenfrac[MAX_SOI_LAY];   //totally frozen: 1, totally unfrozen: 0, partially frozen: 0 - 1.0
